...
//     ${isMobile ? 'scale-[0.85] origin-top' : 'scale-100'}
//   `}
//     >
...
export default function InteractiveCardsSection() {
  ...
  useEffect(() => {
    [1, 2, 3, 4].forEach((set) => {
      ['left', 'center', 'right'].forEach((pos) => {
        ...
        img.src = `/Design/productionReady-card/cards-sets/card-set-${set}/card-${pos}-${set}.png`;
      });
    });
  }, []);
  ...
  return (
    <section
      className={`
        relative flex flex-col items-center justify-center overflow-visible w-full mt-[48px]
        ${isMobile ? 'scale-[0.85] origin-top' : 'scale-100'}
      `}
    >
      {/* Верхние 3 карточки — с AnimatePresence, чтобы не было моргания */}
      <div className="flex items-center justify-center mb-[24px]">
      ...
      {/* Нижняя карусель — 100% как у тебя было */}
      <div className="relative w-full flex justify-center overflow-visible">
        <div
          className="relative flex items-end justify-center"
          style={{
